      ...
      {menuItems.map((item) => {
        ...
        return (
          <Link
            key={item.href}
            href={item.href}
            className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-xs transition-all duration-200 group ${
              isActive
                ? 'bg-primary text-white font-bold shadow-md shadow-primary/20 ring-1 ring-primary/40'
                : 'text-foreground/75 hover:text-primary hover:bg-primary/5 font-medium'
            }`}
          >
            <div className="flex items-center gap-3 min-w-0">
              <svg 
                className={`w-4 h-4 shrink-0 fill-none stroke-current transition-transform duration-200 ${
                  isActive ? 'text-white scale-110' : 'text-foreground/60 group-hover:text-primary'
                }`} 
                viewBox="0 0 24 24" 
            ...
            {isActive && (
              <span className="w-1.5 h-1.5 rounded-full bg-white shrink-0 ml-2 animate-pulse" />
            )}
          </Link>
        );
      })}
